...
class RTSPTestClient extends IElementWatcher  {
    ...
    constructor(webstreamer,name) {
        ...
        this.spectrum={
            ...
            threshold: -80, //dB threshold for result. All lower values will be set to this
        }
    }


    startup(){
        // this.launch = '(audiotestsrc ! audioconvert ! audio/x-raw, rate=32000 ! spectrum name=spectrum bands=128 ! spectrascope ! autovideosink)'
        this.launch = 'rtspsrc location=rtsp://127.0.0.1/test name=r r. ! rtph264depay ! decodebin  ! autovideosink sync=false r. ! rtppcmadepay ! decodebin3 ! audioconvert ! audioresample ! audio/x-raw, rate=32000 ! spectrum name=spectrum bands=128 ! spectrascope ! autovideosink sync=false'
        this.elements ={
            spectrum:{
                type : 'spectrum'
            }
